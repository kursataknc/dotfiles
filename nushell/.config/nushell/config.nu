...
#   1. Theme Definitions (dark/light)
#   2. Core Settings ($env.config)
#   3. Keybindings & Menus
#   4. Custom Aliases & Functions
...
# ==============================================================================
# 2. CORE CONFIGURATION ($env.config)
# ==============================================================================
...
$env.config = {
    ...
    keybindings: [
        ...
        # --- FZF bindings (parity with zsh fzf shell extensions) ---
        {
            ...
            event: {
                ...
                cmd: "commandline edit --insert (fd --type f --hidden --follow --exclude .git | fzf --height 40% --reverse | str trim)"
            }
        }
        {
            name: fzf_cd
            ...
            event: {
                send: executehostcommand
                cmd: "cd (fd --type d --hidden --follow --exclude .git | fzf --height 40% --reverse | str trim)"
            }
        }
    ]
}
...
# ==============================================================================
# 4. CUSTOM FUNCTIONS & ALIASES
# ==============================================================================

# Custom function: change directory and list
def --env cx [arg] {
    cd $arg
    ls -l
}
...
# ------------------------------------------------------------------------------
# Shared aliases live in aliases.nu (loaded below).
...
# Load shared aliases (kept in sync with zshrc/.config/zsh/aliases.zsh)
source ~/.config/nushell/aliases.nu

# Tool init files generated by install.sh
# Regenerate manually when the tool updates, e.g.:
